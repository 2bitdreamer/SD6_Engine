...
void FireEvent(const std::string& eventName, NamedProperties& args /*= NamedProperties()*/, bool fromConsole)
{
	EventSystem& eventSys = EventSystem::GetInstance();
	eventSys.FireEvent(eventName, args, fromConsole);
}

void FireEvent(const std::string& eventName, bool fromConsole)
{
	NamedProperties np;
	EventSystem& eventSys = EventSystem::GetInstance();
	eventSys.FireEvent(eventName, np, fromConsole);
}
